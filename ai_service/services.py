...
class AiAgentService:
    def __init__(self):
        ...
        self.tool_policy_prompt = getattr(
            ...
            (
                ...
                "9) 凡用户问题涉及可从 tushare 查询的**具体事实**（股票代码或名称、价格、涨跌幅、"
                "成交量额、换手率、排名与榜单、财报科目数值、某交易日的截面或时序行情等），"
                "必须在输出具体数字或表格**之前**先调用相应工具并取得返回；禁止仅凭训练记忆、"
                "行业常识或「典型龙头」印象编造或补全数据。工具返回为空、字段缺失或报错时，"
                "如实说明原因与局限，不要捏造数字充数。\n"
                "10) 不要向用户声称「已调用某工具」「正在拉取某接口」或展示看似来自接口的表格，"
                "除非本轮对话中确实执行了对应工具且回答与工具返回一致；避免叙述性伪造工具过程。"
            ),
        )
    ...
    def _build_stream_system_prompt(
        ...
        instruction = (
            ...
            "可直接用自然语言回答、**不要**输出上述代码块的情形：与具体证券代码/报价无关的概念解释、"
            "方法论、合规与风险提示等。**凡需要给出或讨论具体证券的可查事实（含任意交易日的行情、"
            "排行、财务数字等），必须先输出上述 JSON 代码块完成工具调用**，在收到工具结果后再组织答案；"
            "禁止在同一轮模型输出中、在尚未发起任何工具调用的情况下，直接给出带具体数值的股票表格或榜单。\n"
