...
def fetch_etf_basic(
    ...
    fields: str | None = 'ts_code,extname,index_code,index_name,exchange,mgr_name,csname,cname,setup_date,list_date,list_status,etf_type,custod_name,mgt_fee',
    token: str | None = None,
) -> dict:
    """
...
def fetch_etf_daily(
    ...
    fields: str | None = 'ts_code,trade_date,open,high,low,close,pre_close,change,pct_chg,vol,amount',
    token: str | None = None,
) -> dict:
    """
...
def _compute_macd_for_group(g: pd.DataFrame, close_col: str = 'close', fast: int = 12, slow: int = 26, signal: int = 9) -> pd.DataFrame:
    ...
    功能：
    - 对单个 ETF 的时间序列（按 trade_date 排序）计算 DIF/DEA/MACD，并生成 *_bfq 列。
    ...
    返回值：
    - DataFrame: 增加 `macd_dif_bfq`, `macd_dea_bfq`, `macd_bfq` 三列后的数据框
...
def load_etf_daily_dataset(days: int = 365, limit: int = 30, token: str | None = None) -> pd.DataFrame:
    """
    批量获取多个 ETF 的日线行情并本地计算 MACD，聚合为训练样本

    功能：
    - 查询ETF基础信息（默认仅上市 L），限定数量后逐只调用 `fund_daily` 拉取日线行情。
    - 本地计算 MACD 指标并生成 `macd_bfq, macd_dif_bfq, macd_dea_bfq` 列。
    - 聚合为一个 DataFrame，包含各ETF的 `ts_code` 与逐日记录，按 `ts_code, trade_date` 排序。

    参数：
    - days: 自然日跨度，默认 365（最近一年）
    - limit: ETF数量上限，默认 30，避免请求过多导致速率限制或耗时过长。
    - token: 可选的 Tushare Token（覆盖环境变量）。

    返回值：
    - pandas.DataFrame：跨多个ETF的训练样本原始数据

    事件：
    - 调用 `etf_basic` 获取ETF列表（仅上市 L） → 调用 `fund_daily` 拉取日线 → 本地计算MACD → 聚合与排序。
    """
    # 1) 获取ETF列表
    basic_resp = fetch_etf_basic(
        list_status='L',
        etf_type='境内',
        fields='ts_code,extname,index_code,index_name,exchange,mgr_name',
        token=token,
    )
    if not isinstance(basic_resp, dict) or basic_resp.get('code') != 200:
        msg = basic_resp.get('message') if isinstance(basic_resp, dict) else str(basic_resp)
        raise RuntimeError(f"获取ETF基础信息失败: {msg}")
    basic_records = basic_resp.get('data', {}).get('records', [])
    basic_records = [r for r in basic_records if str(r.get('ts_code') or '').strip().startswith('56')]
    if not basic_records:
        raise RuntimeError('未获取到任何ETF基础信息记录')
    ...
    for r in basic_records:
        ...
        df_one = pd.DataFrame(records).sort_values('trade_date').reset_index(drop=True)
        df_one = _compute_macd_for_group(df_one)
        all_records.append(df_one)

    if not all_records:
        raise RuntimeError('未获取到任何ETF的日线行情数据')
...
def scan_all_etf_recent_growth(
    limit: int = 1500,
    forecast_days: int = 5,
    ...
    """
    使用已训练的 MACD XGBoost 上涨模型，批量扫描所有 ETF 的“最近一天”是否存在未来5天内上涨≥5%的可能

    功能：
    - 加载本地保存的 `macd_etf_xgb_up.joblib` 模型组件（模型/标准化器/特征列/参数）。
    - 遍历全部或指定数量的 ETF 代码，拉取最近一段时间的基础行情并本地计算 MACD。
    - 针对每个 ETF 的“最近一个交易日”样本进行预测，若预测类别为上涨事件（类别=1），打印并收集结果。
    ...
    """
    # 加载模型文件
    model_path = Path(__file__).resolve().parent / "models" / "macd_etf_xgb_up.joblib"
    ...
    # 获取 ETF 基础信息列表（仅上市），参考 load_etf_daily_dataset 的实现
    basic_resp = fetch_etf_basic(
        list_status='L',
        etf_type='境内',
        fields='ts_code,extname,index_name,exchange',
        token=token,
    )
    if not isinstance(basic_resp, dict) or basic_resp.get('code') != 200:
        msg = basic_resp.get('message') if isinstance(basic_resp, dict) else str(basic_resp)
        raise RuntimeError(f"获取ETF基础信息失败: {msg}")
    basic_records = basic_resp.get('data', {}).get('records', [])
    basic_records = [r for r in basic_records if str(r.get('index_name') or '').strip()]
    ...
    # 构建 ETF 代码信息列表（统一 name / market 字段）
    code_list = []
    for r in basic_records:
        ts_code = r.get('ts_code')
        if not ts_code:
    ...
    # 时间范围与字段（对标 ETF 拉取逻辑：仅取基础行情列，MACD 本地计算）
    end_date = datetime.now().strftime('%Y%m%d')
    ...
    for code in code_list:
        # 使用 ETF 日线接口，保持与 load_etf_daily_dataset 一致
        resp = fetch_etf_daily(
        ...
        # 排序并本地计算 MACD 指标，保持与 ETF 处理逻辑一致
        df_one = pd.DataFrame(records).sort_values('trade_date').reset_index(drop=True)
        df_one = _compute_macd_for_group(df_one)

        processed = predictor.prepare_features(df_one, for_inference=True)
        if processed.empty:
            continue
        ...
        if int(y_pred) == 1:
            ...
            # 打印命中结果
            print(
                f"命中: 市场 {code['market']} ETF {code['ts_code']}-{code['name']} 在 {last_date} 存在未来{forecast_days}天上涨≥{growth_threshold*100:.0f}% 的可能，"
    ...
    header_html = (
        ...
        f"<h2 style=\"margin:0 0 8px;\">MACD XGBoost ETF扫描结果【此数据仅作参考，不构成任何交易建议】</h2>"
        ...
        f"<p style=\"margin:0 0 12px;\">共扫描 {scanned_count} 只ETF，命中 {hit_count} 只</p>"
        f"</div>"
    )
...
def main_predict(
    ...
    df = load_etf_daily_dataset(days=days, limit=total_limit)
    ...
    if len(codes) <= 5:
        ...
        try:
            ...
            model_path = model_dir / f"macd_etf_xgb_{predictor.target_mode}.joblib"
            joblib.dump({
...
def batch_test_macd_xgboost():
    ...
    print(f"批量测试：拉取 {total_limit} 只ETF的数据用于参数网格评估...")
    df = load_etf_daily_dataset(days=365, limit=total_limit)
...
if __name__ == "__main__":
    ...
    # main_predict()
    hit = scan_all_etf_recent_growth()
    print(hit['hits'])
