...
class IndustryUpDownRatioStrategyTests(unittest.TestCase):
    """
    组件：行业涨跌比例策略测试。

    功能：
    - 验证基于 `dc_index` 的上涨家数、下跌家数计算是否正确。
    ...
    @patch("stock_strategy.industry_up_down_ratio_strategy.cache")
    @patch("stock_strategy.industry_up_down_ratio_strategy.get_open_trade_dates")
    @patch("stock_strategy.industry_up_down_ratio_strategy.call_tushare")
    def test_get_industry_up_down_ratio_returns_board_ratios(
        self,
        mock_call_tushare,
        mock_get_open_trade_dates,
        mock_cache,
        ...
        def _side_effect(interface, params=None, fields=None, token=None, use_query=False):
            ...
            if interface != "dc_index":
                raise AssertionError(f"unexpected interface: {interface}")
            self.assertEqual(params["idx_type"], "行业板块")
            self.assertEqual(fields, "ts_code,trade_date,name,idx_type,level,up_num,down_num")
            records_by_date = {
                "20260102": [
                    {
                        "ts_code": "BK001.DC",
            ...
            return {
                "code": 200,
                "data": {"records": records_by_date[params["trade_date"]]},
            }

        mock_call_tushare.side_effect = _side_effect
        strategy = IndustryUpDownRatioStrategy()

        result = strategy.get_industry_up_down_ratio(
            start_date="2026-01-02",
            end_date="2026-01-03",
            idx_type="行业板块",
        )
    ...
    @patch("stock_strategy.industry_up_down_ratio_strategy.get_open_trade_dates")
    @patch("stock_strategy.industry_up_down_ratio_strategy.call_tushare")
    def test_get_industry_up_down_ratio_filters_by_dc_industry_level(
        self,
        mock_call_tushare,
        mock_get_open_trade_dates,
        mock_cache,
        ...
        """
        功能：验证传入东财行业层级时，只返回指定 level 的板块涨跌比例结果。
